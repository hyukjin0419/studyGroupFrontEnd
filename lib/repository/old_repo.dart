...
//   }
//
//   void _restorePendingFakes(Map<int, List<MemberChecklistItemVM>> fakeItemsByMember) {
//     if (fakeItemsByMember.isEmpty) return;
//
//     int restoredCount = 0;
//
//     for (final entry in fakeItemsByMember.entries) {
//       final memberId = entry.key;
//       final fakeItems = entry.value;
//
//       // 해당 멤버의 그룹 찾기
//       final group = _groups.firstWhereOrNull(
//               (g) => g.studyMemberId == memberId
//       );
//
//       if (group == null) continue;
//
//       // ✅ 실제 데이터의 content들을 Set으로 (O(1) 검색)
//       final realContents = group.items
//           .where((item) => item.id > 0)
//           .map((item) => item.content.trim().toLowerCase())
//           .toSet();
//
//       // fake 중에서 실제 데이터에 없는 것만 추가
//       for (final fakeItem in fakeItems) {
//         final normalizedContent = fakeItem.content.trim().toLowerCase();
//
//         if (!realContents.contains(normalizedContent)) {
//           group.items.add(fakeItem);
//           restoredCount++;
//           log("🔄 [Provider] fake 복원: ${fakeItem.content}");
//         } else {
//           log("✨ [Provider] fake → 실제 교체: ${fakeItem.content}");
//         }
//       }
//     }
//
//     if (restoredCount > 0) {
//       log("📝 [Provider] 총 ${restoredCount}개 fake 복원 (아직 대기중)");
//     }
//   }
// }